...
import {
  Menu, Search, Sun, Bell, Zap, ChevronDown, RefreshCw, ExternalLink,
  Smartphone, Radio, CheckCircle2, XCircle, AlertTriangle, Info,
...
const FIELD_MAP = {
  url:      [{key:"url",   label:"URL / Link",    placeholder:"https://eyerexus.com/new-page",   required:true},
             {key:"title", label:"Title",          placeholder:"EyeRexUs – New Experience",       required:false},
             {key:"desc",  label:"Description",    placeholder:"Updated link to connect...",      required:false, multiline:true}],
...
function HowItWorksModal({ onClose }) {
  const steps = [
    {n:1,Icon:Radio,        title:"Scan Card",          desc:"Hold your NFC card near the device to read current data."},
    {n:2,Icon:Activity,     title:"Read Current Data",  desc:"The app reads and displays all data stored on the card."},
    {n:3,Icon:FileText,     title:"Update Data",        desc:"Select a new data type and enter the updated information."},
    {n:4,Icon:CheckCircle2, title:"Write & Verify",     desc:"Tap the card again to write new data and verify success."},
  ];
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white border border-red-200 rounded-2xl p-6 w-full max-w-md shadow-2xl shadow-red-100">
        ...
        <button onClick={onClose} className="w-full mt-5 bg-red-600 hover:bg-red-500 text-white text-sm font-bold py-2.5 rounded-xl transition-all shadow-md shadow-red-200">Got it!</button>
      </div>
    </div>
  );
}
...
export default function ReWriteNFCCardPage({ onMenuClick }) {
  const [activeStep,  setActiveStep]  = useState(1);
  const [selType,     setSelType]     = useState(0);
  const [fieldVals,   setFieldVals]   = useState({url:"https://eyerexus.com/new-page",title:"EyeRexUs – New Experience",desc:"Updated link to connect and explore new opportunities."});
  ...
  const handleWrite = useCallback(async () => {
    if (!primVal.trim()) { showToast("Please enter required data first.","error"); return; }
    if (typeof NDEFReader==="undefined") {
        ...
        <div className="p-3 sm:p-5">
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-red-900 font-bold text-xl sm:text-2xl">Re-Write NFC Card</h1>
                <p className="text-red-400 text-xs sm:text-sm mt-0.5 hidden sm:block">Rewrite or update data on your existing NFC card</p>
              </div>
            </div>
          ...
          {/* Main 3-column grid → stacked on mobile */}
          <div className="flex flex-col xl:flex-row gap-4">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              ...
              <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-red-900 font-semibold text-sm flex items-center gap-2">
                    <CreditCard className="w-4 h-4 text-red-500"/>Current Card Information
                  </h2>
                ...
                {showCard ? (
                  ...
                  <div className="flex flex-col items-center py-4 gap-3">
                    ...
                    <button onClick={handleScanCard}
                      ...
                      <Radio className="w-4 h-4"/>Scan NFC Card
                    </button>
                  </div>
                )}
                ...
                {showCard && (
                  <div className="mt-4 pt-4 border-t border-red-100">
                    <h3 className="text-red-900 text-xs font-semibold mb-3">Current Data Preview</h3>
                    ...
                    <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-2 flex items-center gap-2 mb-2">
                      <span className="text-red-700 text-xs flex-1 truncate">https://eyerexus.com/your-profile</span>
                      <button className="text-red-400 hover:text-red-600 transition-colors flex-shrink-0">
                        <ExternalLink className="w-3.5 h-3.5"/>
                      </button>
                    </div>
                    <div className="text-red-400 text-xs">Title: <span className="text-red-700">EyeRexUs – Tap. Impress. Grow.</span></div>
                    <div className="text-red-400 text-xs mt-0.5">Description: <span className="text-red-700">Connect instantly and explore more.</span></div>
                    <button onClick={()=>showToast("Loading full card data...")}
                      className="w-full mt-3 bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 hover:text-red-700 text-xs font-semibold py-2 rounded-lg transition-all">
                      View Full Data
                    </button>
                  </div>
                )}
              ...
              {/* Write Statistics + Charts */}
              <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
                <h2 className="text-red-900 font-semibold text-sm mb-4 flex items-center gap-2">
                  <BarChart2 className="w-4 h-4 text-red-500"/>Write Statistics
                </h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <p className="text-red-500 text-[10px] font-semibold mb-1">Success Rate Trend</p>
                    <ResponsiveContainer width="100%" height={80}>
            ...
            <div className="w-full xl:w-[460px] flex-shrink-0 space-y-4">
              <div className="bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm">
                ...
                <p className="text-red-400 text-xs mb-4">Select data type and update the information you want to write</p>

                {/* Data Type Grid — 3 cols mobile → 4 cols sm */}
              ...
              {nfcStatus!==NFC.IDLE && (
                <div className={`flex items-center gap-2 text-xs font-medium px-4 py-2.5 rounded-xl border ${
                  ...
                  {nfcStatus===NFC.SCANNING    &&"Scanning for NFC card... Hold card near the device."}
                  {nfcStatus===NFC.WRITING     &&"Writing data to NFC card... Do not remove the card."}
                  {nfcStatus===NFC.SUCCESS     &&"Card rewritten and verified successfully!"}
                  {nfcStatus===NFC.ERROR       &&"Write failed. Check card compatibility and try again."}
                  {nfcStatus===NFC.UNSUPPORTED &&"Web NFC not supported. Running in demo mode."}
                </div>
              )}
            </div>
            ...
            <div className="w-full xl:w-60 flex-shrink-0 space-y-4">
              ...
              <div className="bg-white border border-red-100 rounded-xl p-4 shadow-sm">
                <h3 className="text-red-900 font-semibold text-sm mb-0.5">Tap Your NFC Card</h3>
                ...
                <div className="flex justify-center mb-4">
                  <div className={`flex items-center gap-1.5 border text-xs font-semibold px-3 py-1.5 rounded-full ${
                    ...
                    {isWriting?"Processing...":nfcStatus===NFC.SUCCESS?"Write Success!":"Ready to Scan"}
                  </div>
                </div>
              ...
              <div className="bg-white border border-red-100 rounded-xl p-4 shadow-sm">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={()=>showToast("Loading all activity...")} className="text-red-500 hover:text-red-700 text-[10px] font-semibold">View All</button>
                </div>
              ...
              <div className="bg-white border border-red-100 rounded-xl p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-red-900 font-semibold text-sm">Important Tips</h3>
                </div>
                <ul className="space-y-1.5">
                  {[
                    "Make sure your card is writable and unlocked.",
                    "Do not remove the card while writing.",
                    "Keep the card steady near the device.",
                    "Some cards may require unlocking before rewrite.",
                  ].map((tip,i)=>(
