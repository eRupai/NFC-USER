...
import {
  Crown, CheckCircle2, X, Settings, XCircle, CreditCard,
  Download, Filter, BarChart2, RefreshCw, Calendar, Users,
  Activity, Lock, ArrowUpRight, ArrowRight, Bell, Zap,
} from "lucide-react";
import {
...
const PLANS = {
  monthly: [
    ...
    { id:"premium",    Icon:Crown,     name:"Premium",    price:15,  tagline:"Best for growing businesses", features:["15,000 Credits / month","Up to 15 Users","All Features Included","Priority Support"],  bg:"from-red-600 to-rose-700"  },
  ...
  yearly: [
    ...
    { id:"premium",    Icon:Crown,     name:"Premium",    price:144, tagline:"Best for growing businesses", features:["15,000 Credits / month","Up to 15 Users","All Features Included","Priority Support"],  bg:"from-red-600 to-rose-700"  },
...
const USAGE_CHART = [
  {day:"Mon",credits:1200},{day:"Tue",credits:1450},{day:"Wed",credits:980},
  {day:"Thu",credits:1800},{day:"Fri",credits:1350},{day:"Sat",credits:620},{day:"Sun",credits:890},
];
...
const USAGE = {
  credits:8742, maxCredits:15000,
  scans:125630, maxScans:200000,
  users:2450,   maxUsers:5000,
};
...
function Toast({ msg, type, onClose }) {
  if (!msg) return null;
  const s = {
    success:"bg-red-600 border-red-400",
    error:  "bg-red-800 border-red-600",
    info:   "bg-rose-600 border-rose-400",
  };
  return (
...
export default function SubscriptionPage({ onMenuClick }) {
  ...
  const handleCancelSub = () => setConfirm({
    ...
    msg:"Are you sure? You'll lose premium features at the end of your billing cycle.",
    onConfirm:()=>{ setConfirm(null); setAutoRenew(false); showToast("Cancellation scheduled.","info"); },
  });

  const handleUpdateCard = () => {
    if (!cardNum || cardNum.length<16) { showToast("Enter a valid 16-digit card number.","error"); return; }
    if (!cardExp)                      { showToast("Enter expiry date.","error"); return; }
  ...
  const handleDownloadAll = () => {
    ...
    a.download = "invoices.txt"; a.click();
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[#fdf5f5]">
      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-5 py-4 sm:py-5">
        ...
        {/* ── Row 1: Current Plan + Usage + Billing Summary ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          <div className={`bg-white border border-red-100 rounded-xl p-4 sm:p-5 shadow-sm transition-opacity ${tab==="history"?"lg:opacity-40 lg:pointer-events-none":""}`}>
            ...
            <p className="text-red-500 text-[10px] font-semibold mb-1">Weekly Credit Usage</p>
            <ResponsiveContainer width="100%" height={80}>
              <LineChart data={USAGE_CHART} margin={{top:2,right:2,left:-32,bottom:0}}>
                ...
                <Tooltip {...TOOLTIP}/>
                <Line type="monotone" dataKey="credits" name="Credits" stroke="#ef4444" strokeWidth={2}
                  dot={{r:2,fill:"#ef4444",stroke:"#fff",strokeWidth:1}}/>
              </LineChart>
            </ResponsiveContainer>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <div>
              <h3 className="text-red-900 font-semibold text-base flex items-center gap-2">
                <Crown className="w-4 h-4 text-red-500"/>Choose Your Plan
              </h3>
              <p className="text-red-400 text-xs mt-0.5">Pick the plan that fits your needs</p>
            </div>
                ...
                <div key={plan.id}
                  ...
                  <button onClick={()=>handleChoosePlan(plan)}
                    className={`w-full py-2 rounded-lg text-xs font-bold transition-all ${
                      isCurrent
                        ?"bg-red-600 text-white cursor-default shadow-sm shadow-red-200"
