...
import {
  Search, Plus, LayoutGrid, List, Filter, Bell, Zap, ChevronDown,
  ...
  HardDrive, TrendingUp, Star, X, MoreVertical, Radio, AlertCircle, Menu,
} from "lucide-react";
import {
    ...
    <div className="bg-white border border-red-100 hover:border-red-300 rounded-xl overflow-hidden transition-all group relative shadow-sm hover:shadow-lg">
      <div className={`relative h-[90px] sm:h-[100px] bg-gradient-to-br ${st.bg}`}>
        ...
        <div className="absolute top-2 right-2">
          ...
          {open && (
            <div className="absolute right-0 top-7 bg-white border border-red-200 rounded-xl shadow-2xl shadow-red-100 overflow-hidden z-10 w-36">
              {[
                {label:"Use Template",action:()=>{onUse(tpl);   setOpen(false);},Icon:CheckCircle2,color:"text-red-600"},
                {label:"Edit",        action:()=>{onEdit(tpl);  setOpen(false);},Icon:Pencil,      color:"text-red-900"},
                {label:saved?"Unsave":"Save",action:()=>{onSaveToggle(tpl);setOpen(false);},Icon:Bookmark,color:"text-rose-600"},
                {label:"Delete",      action:()=>{onDelete(tpl);setOpen(false);},Icon:Trash2,      color:"text-red-500"},
              ].map(m=>(
          ...
          <>
            <div>
              ...
              <div className="relative">
                ...
                <input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Business Card Template"
        ...
        {step===4 && (
          <div className="space-y-3">
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-3">
              ...
              {["Choose the right data type for your need.","Dynamic data can be updated later.","Lock after write for security.","Preview before saving template."].map(t=>(
...
export default function NFCTemplatesPage({ onMenuClick }) {
  ...
  const filtered = useMemo(()=>{
    let list = sideTab==="mine"?templates.filter(t=>t.mine):templates;
    ...
    return list;
  },[templates,search,catFilter,typeFilter,pill,sortBy,sideTab]);
  ...
  const handleSaveToggle=(tpl)=>{setSaved(p=>{const n=new Set(p);n.has(tpl.id)?(n.delete(tpl.id),showToast(`"${tpl.name}" unsaved.`,"info")):(n.add(tpl.id),showToast(`"${tpl.name}" saved!`));return n;});};
  const handleCreateSave=(data)=>{
    if(editTpl){setTemplates(p=>p.map(t=>t.id===editTpl.id?{...t,...data}:t));showToast(`"${data.name}" updated!`);setEditTpl(null);}
    else{setTemplates(p=>[{id:Date.now(),...data,uses:0,saves:0,mine:true},...p]);showToast(`"${data.name}" created!`);}
      ...
      <div className="flex-1 overflow-hidden flex relative">
        ...
        <div className="flex-1 overflow-y-auto px-3 sm:px-5 py-4 sm:py-5 min-w-0">
          ...
          <div className="flex items-start justify-between mb-3 sm:mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-red-400 text-xs hidden sm:block mt-0.5">Create, manage and reuse your NFC card templates</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={()=>setPanelOpen(p=>!p)}
                className="lg:hidden flex items-center gap-1.5 bg-white border border-red-200 hover:border-red-400 text-red-600 hover:text-red-700 text-xs font-semibold px-3 py-2 rounded-xl transition-all shadow-sm">
          ...
          <div className="flex items-center gap-1 mb-3 sm:mb-4 bg-white border border-red-200 rounded-lg p-1 w-fit overflow-x-auto shadow-sm">
            {[{id:"all",label:"All"},{id:"mine",label:"My Templates"},{id:"create",label:"Create"},{id:"import",label:"Import"}].map(t=>(
              <button key={t.id} onClick={()=>{setSideTab(t.id);if(t.id==="create")setPanelOpen(true);}}
                className={`px-2.5 sm:px-3 py-1.5 rounded-md text-xs font-semibold transition-all whitespace-nowrap ${sideTab===t.id?"bg-red-600 text-white shadow-sm":"text-red-500 hover:text-red-700"}`}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          {sideTab==="create" ? (
            <div className="bg-white border border-red-200 rounded-xl p-4 sm:p-5 max-w-lg shadow-md">
              <CreateTemplatePanel onSave={handleCreateSave} editData={editTpl} onReset={()=>{setEditTpl(null);setSideTab("all");}}/>
            </div>
          ) : (
            <>
              {/* Filters */}
              ...
              {paginated.length===0 ? (
                <div className="text-center py-16">
                  ...
                  <div className="text-red-400 text-sm">Try adjusting your filters or create a new template</div>
                </div>
              ) : viewMode==="grid" ? (
          ...
          <div className="mt-5 space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
              {[
                {Icon:FileText,    bg:"from-red-500 to-rose-600",   label:"Total Templates",value:totalTemplates,     sub:`+${templates.filter(t=>t.mine).length} mine`},
                {Icon:Star,        bg:"from-rose-500 to-red-600",   label:"Most Used",      value:mostUsed?.name||"-",sub:`${fmt(mostUsed?.uses||0)} uses`},
                {Icon:Pencil,      bg:"from-red-600 to-rose-700",   label:"Total Writes",   value:fmt(totalWrites),   sub:"+32.5% this month"},
                {Icon:HardDrive,   bg:"from-rose-600 to-red-800",   label:"Storage Used",   value:"42.6 MB",          sub:"of 100 MB"},
                {Icon:CheckCircle2,bg:"from-red-700 to-rose-600",   label:"Success Rate",   value:"98.75%",           sub:"Excellent"},
              ].map(s=>(
                <div key={s.label} className="bg-white border border-red-100 rounded-xl p-3 flex items-center gap-2 sm:gap-3 shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-red-900 font-bold text-xs sm:text-sm leading-tight truncate">{s.value}</div>
        ...
        <div className="w-72 flex-shrink-0 border-l border-red-100 bg-white overflow-y-auto px-4 py-5 hidden lg:flex flex-col shadow-lg">
          <CreateTemplatePanel onSave={handleCreateSave} editData={editTpl} onReset={()=>setEditTpl(null)}/>
        </div>
        ...
        {panelOpen && (
          <div className="lg:hidden fixed inset-0 z-40 flex">
            ...
            <div className="w-80 bg-white border-l border-red-200 flex flex-col overflow-y-auto px-4 py-5 shadow-2xl">
              ...
              <CreateTemplatePanel onSave={handleCreateSave} editData={editTpl} onReset={()=>{setEditTpl(null);setPanelOpen(false);}}/>
            </div>
          </div>
        )}
      </div>
