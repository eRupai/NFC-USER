...
import {
  Menu, Search, Sun, Bell, Zap, Calendar, Download,
  CreditCard, Radio, Users, Star, BarChart2, CheckCircle2,
  MapPin, Smartphone, Clock, Globe, ArrowUpRight, ChevronDown, X,
} from "lucide-react";
import {
...
const SCAN_DAILY   = [{l:"May 1",v:800},{l:"May 6",v:1400},{l:"May 11",v:1100},{l:"May 16",v:1600},{l:"May 21",v:1300},{l:"May 26",v:1900},{l:"May 31",v:2100}];
const SCAN_WEEKLY  = [{l:"W1",v:5200},{l:"W2",v:6800},{l:"W3",v:6100},{l:"W4",v:7530}];
const SCAN_MONTHLY = [{l:"Jan",v:18000},{l:"Feb",v:21000},{l:"Mar",v:19500},{l:"Apr",v:23000},{l:"May",v:25630}];

const ENGAGE_DATA = [
  {l:"May 1",  newUsers:280, returning:520, inactive:200},
  {l:"May 6",  newUsers:320, returning:680, inactive:280},
  {l:"May 11", newUsers:290, returning:610, inactive:240},
  {l:"May 16", newUsers:380, returning:760, inactive:310},
  {l:"May 21", newUsers:340, returning:700, inactive:270},
  {l:"May 26", newUsers:410, returning:810, inactive:290},
  {l:"May 31", newUsers:430, returning:850, inactive:260},
];

const OS_DATA = [
  {l:"May 1",  android:520, ios:210, other:70 },
  {l:"May 6",  android:680, ios:280, other:90 },
  {l:"May 11", android:610, ios:250, other:80 },
  {l:"May 16", android:760, ios:310, other:100},
  {l:"May 21", android:700, ios:290, other:90 },
  {l:"May 26", android:810, ios:330, other:110},
  {l:"May 31", android:850, ios:350, other:120},
];

const TOP_LOCATIONS = [
  {flag:"🇮🇳",country:"India",         scans:7245},
  {flag:"🇺🇸",country:"United States", scans:4582},
  {flag:"🇬🇧",country:"United Kingdom",scans:2154},
  {flag:"🇨🇦",country:"Canada",        scans:1245},
  {flag:"🇦🇺",country:"Australia",     scans:982},
  {flag:"🇩🇪",country:"Germany",       scans:824},
  {flag:"🇦🇪",country:"UAE",           scans:632},
  {flag:"🌍",country:"Others",         scans:3966},
];

const TOP_CARDS = [
  {name:"Business Card",scans:4521,pct:100},
  {name:"Website Link", scans:3245,pct:72},
  {name:"Product Info", scans:2987,pct:66},
  {name:"Social Media", scans:2345,pct:52},
  {name:"Event Details",scans:1987,pct:44},
];

const DEVICES_PIE  = [{name:"Android",value:68.2,fill:"#ef4444"},{name:"iOS",value:27.6,fill:"#f87171"},{name:"Other",value:4.2,fill:"#fecaca"}];
const USER_TYPE_PIE= [{name:"New Users",value:28.1,fill:"#ef4444"},{name:"Returning",value:45.7,fill:"#f87171"},{name:"Inactive",value:26.2,fill:"#fecaca"}];
...
export default function ScanAnalyticsPage({ onMenuClick }) {
  ...
  const handleExport = () => {
    const rows = TOP_LOCATIONS.map(l=>`${l.country},${l.scans}`);
    const blob = new Blob([["Country,Scans",...rows].join("\n")],{type:"text/csv"});
    const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="scan_analytics.csv";a.click();
    showToast("Analytics exported!");
  };

  const TABS = [
    {id:"scan",     label:"Scan Analytics",    Icon:Radio},
    {id:"engage",   label:"User Engagement",   Icon:Users},
    {id:"location", label:"Location",          Icon:MapPin},
    {id:"device",   label:"Device Analytics",  Icon:Smartphone},
  ];

  const STATS = [
    {Icon:CreditCard,  bg:"from-red-500 to-rose-600",   label:"Total Scans",      value:"25,630", change:"32.7%"},
    {Icon:Radio,       bg:"from-rose-500 to-red-600",   label:"Unique Scans",     value:"18,742", change:"28.4%"},
    {Icon:Users,       bg:"from-red-600 to-rose-700",   label:"Unique Users",     value:"8,721",  change:"21.4%"},
    {Icon:Star,        bg:"from-rose-600 to-red-800",   label:"New Users",        value:"2,450",  change:"18.6%"},
    {Icon:BarChart2,   bg:"from-red-700 to-rose-600",   label:"Avg. Scan / Day",  value:"827",    change:"14.3%"},
    {Icon:CheckCircle2,bg:"from-rose-700 to-red-700",   label:"Success Rate",     value:"98.75%", change:"8.6%"},
  ];
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-3 sm:px-5 py-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-red-900 font-bold text-xl sm:text-2xl">Scan &amp; Analytics</h1>
              <p className="text-red-400 text-xs hidden sm:block">Track NFC card scans and user engagement</p>
            </div>
          </div>
        ...
        {/* Stats — 2 cols → 3 sm → 6 xl — compact padding on mobile */}
        ...
        {/* Row 1: Scan Chart + Top Locations — stack → side-by-side lg */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                <h3 className="text-red-900 font-semibold text-sm flex items-center gap-2">
                  <Radio className="w-4 h-4 text-red-500"/>Scan Overview
                </h3>
                <div className="flex items-center gap-1.5 mt-1">
                  <div className="w-2 h-2 rounded-full bg-red-500"/>
                  <span className="text-red-400 text-[10px]">Scans</span>
                </div>
              </div>
          ...
          <div className={`bg-white border border-red-100 rounded-xl p-4 shadow-sm transition-opacity ${tab!=="scan"&&tab!=="location"?"opacity-40 pointer-events-none":""}`}>
            ...
            {/* Stack map and list vertically on mobile, side-by-side on sm */}
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              {/* Location list — 2-col grid on mobile for compactness */}
              <div className="w-full sm:w-48 flex-shrink-0">
                ...
                <button onClick={()=>showToast("Loading full location report...")}
                  className="w-full mt-2 bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 hover:text-red-700 text-[10px] font-semibold py-1.5 rounded-lg transition-all">
                  View Full Report
                </button>
              </div>
            </div>
        ...
        {/* Row 2: Engagement + Top Cards + User Types — 1 col → 2 sm → 3 lg */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
          ...
          <div className={`bg-white border border-red-100 rounded-xl p-4 shadow-sm transition-opacity ${tab==="device"?"opacity-40 pointer-events-none":""}`}>
            <h3 className="text-red-900 font-semibold text-sm mb-3 flex items-center gap-2">
              <Users className="w-4 h-4 text-red-500"/>User Engagement Overview
            </h3>
            <div className="flex items-center gap-3 mb-3">
              <div className="relative flex-shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  ...
                  <span className="text-red-400 text-[8px]">Users</span>
                </div>
              </div>
            ...
            <button onClick={()=>showToast("Loading engagement details...")}
              className="w-full bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 text-xs font-semibold py-2 rounded-lg transition-all">
              View Details
            </button>
          </div>
          ...
          <div className={`bg-white border border-red-100 rounded-xl p-4 shadow-sm transition-opacity ${tab==="device"||tab==="location"?"opacity-40 pointer-events-none":""}`}>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-red-900 font-semibold text-sm">Engagement Over Time</h3>
            ...
            <ResponsiveContainer width="100%" height={140}>
              <LineChart data={ENGAGE_DATA} margin={{top:4,right:4,left:-20,bottom:0}}>
                ...
                <YAxis tick={{fontSize:8,fill:"#f87171"}} axisLine={false} tickLine={false}/>
                <Tooltip {...TOOLTIP_STYLE} itemStyle={{color:"#ef4444"}}/>
                <Line type="monotone" dataKey="newUsers"  name="New Users"  stroke="#ef4444" strokeWidth={2} dot={{r:2.5,fill:"#ef4444",stroke:"#fff",strokeWidth:1.5}}/>
          ...
          <div className={`bg-white border border-red-100 rounded-xl p-4 shadow-sm transition-opacity ${tab==="location"?"opacity-40 pointer-events-none":""}`}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-red-900 font-semibold text-sm flex items-center gap-2">
                <CreditCard className="w-4 h-4 text-red-500"/>Top Scanned Cards
              </h3>
              <button onClick={()=>showToast("Loading all cards...")} className="text-red-500 hover:text-red-700 text-[10px] font-semibold">View All</button>
            </div>
                ...
                <div key={c.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-red-900 text-xs font-medium">{c.name}</span>
        ...
        {/* Row 3: Device Donut + OS Bar Chart + Quick Insights — stack → 3 lg */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className={`bg-white border border-red-100 rounded-xl p-4 shadow-sm transition-opacity ${tab==="engage"||tab==="location"?"opacity-40 pointer-events-none":""}`}>
            <h3 className="text-red-900 font-semibold text-sm mb-3 flex items-center gap-2">
              <Smartphone className="w-4 h-4 text-red-500"/>Device Analytics
            </h3>
            <div className="flex items-center gap-3 mb-3">
              <div className="relative flex-shrink-0">
                <ResponsiveContainer width={110} height={110}>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-red-900 font-bold text-xs">25,630</span>
            ...
            <button onClick={()=>showToast("Loading device details...")}
              className="w-full bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 text-xs font-semibold py-2 rounded-lg transition-all">
              View Details
            </button>
          </div>

          {/* OS Breakdown — Stacked Bar */}
          <div className={`bg-white border border-red-100 rounded-xl p-4 shadow-sm transition-opacity ${tab==="engage"||tab==="location"?"opacity-40 pointer-events-none":""}`}>
            <div className="flex items-center justify-between mb-2">
          ...
          <div className="bg-white border border-red-100 rounded-xl p-4 shadow-sm">
            <h3 className="text-red-900 font-semibold text-sm mb-3 flex items-center gap-2">
              <Star className="w-4 h-4 text-red-500"/>Quick Insights
            </h3>
            <div className="grid grid-cols-2 gap-2 mb-3">
              {[
                {Icon:Calendar, bg:"bg-red-100",  ic:"text-red-600",  label:"Peak Day",   val:"May 20",  sub:"1,852 Scans"},
                {Icon:Clock,    bg:"bg-rose-100", ic:"text-rose-600", label:"Peak Hour",  val:"2:00 PM", sub:"1,245 Scans"},
                {Icon:Globe,    bg:"bg-red-100",  ic:"text-red-600",  label:"Top Country",val:"India",   sub:"7,245 Scans"},
                {Icon:Smartphone,bg:"bg-rose-100",ic:"text-rose-600", label:"Top Device", val:"Android", sub:"68.2% Scans"},
              ].map(q=>(
            ...
            <div className="space-y-2 border-t border-red-100 pt-3">
              {[
                {label:"Avg. Session Duration",val:"3m 24s"},
                {label:"Bounce Rate",          val:"12.4%"},
                {label:"Repeat Scan Rate",     val:"45.7%"},
                {label:"Cards with 0 Scans",   val:"126"},
              ].map(r=>(
